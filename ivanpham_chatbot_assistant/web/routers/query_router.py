...
from ivanpham_chatbot_assistant.services.pipelines.online import (
    online_pipeline as query_service,
)
...
async def ask_question(request: AskRequest):
    ...
    return success_response(
        data=result.model_dump(), message="Query executed successfully"
    )
...
async def query_detail(
    ...
    return success_response(
        data=detail.model_dump(), message=f"Query {query_id} retrieved successfully"
    )
