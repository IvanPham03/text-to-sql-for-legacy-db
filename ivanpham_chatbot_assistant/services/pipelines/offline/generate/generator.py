...
class DescriptionGenerator:
    ...
    def __init__(
        self, llm_service: LLMService, templates_dir: str, max_concurrency: int = 5
    ):
    ...
    async def generate_table_description(
        ...
        try:
            async with self.semaphore:
                ...
                logger.info(
                    f"Table description generated for {table_name} in {latency:.2f}s"
                )
                return description
    ...
    async def generate_column_descriptions(
        self, table_name: str, table_description: str, columns: list[dict[str, Any]]
    ) -> dict[str, str]:
        """
        ...
        for col in columns:
            ...
            tasks.append(
                self._generate_single_column_description(
    ...
    async def _generate_single_column_description(
        ...
        try:
            async with self.semaphore:
                ...
                latency = time.perf_counter() - start_time
                logger.debug(
                    f"Column description generated for {table_name}.{col_name} in {latency:.2f}s"
                )
                return col_name, desc
        except Exception as e:
            logger.warning(
                f"Failed to generate column description for {table_name}.{col_name}: {e}"
            )
