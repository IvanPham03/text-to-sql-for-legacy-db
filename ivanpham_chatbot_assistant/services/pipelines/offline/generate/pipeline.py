...
from ivanpham_chatbot_assistant.services.pipelines.offline.generate.generator import DescriptionGenerator
        ...
        async with self.session_factory() as session:
            ...
            for table in tables:
                try:
                    ...
                    generated_table_desc = await self.generator.generate_table_description(
                        table.name, 
                        cols_data
                    )
                    ...
                    logger.info(f"Semantic descriptions persisted for table: {table.name}")
                except Exception as e:
                    await session.rollback()
                    logger.error(f"Failed to generate/persist descriptions for table {table.name}: {e}")
