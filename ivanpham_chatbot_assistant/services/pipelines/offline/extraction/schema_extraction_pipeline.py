...
class SchemaExtractionPipeline:
    ...
    async def run(self, request: SchemaIndexRequest) -> str:
        ...
        logger.info(
            f"database indexing started for {settings.sql_source_base} on {settings.sql_source_host}"
        )
        ...
        try:
            ...
            logger.info(
                "Adaptive Sampling {count} tables in parallel (ASC + DESC)...",
                count=len(tables),
            )
            ...
            def table_sample_worker(t):
                ...
                try:
                    ...
                    rows = crawler.sample_table_rows(
                        t["schema_name"], t["table_name"], limit=100
                    )
                    ...
                    if not any_samples_found:
                        logger.info(
                            f"Table {table_key} looks sparse (no samples in first 200 rows). Falling back to FULL TABLE SCAN (Unlimited rows)..."
                        )
                        rows = crawler.sample_table_rows(
                            t["schema_name"], t["table_name"], limit=None
                        )
            ...
            with ThreadPoolExecutor(max_workers=24) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if rows:
                        ...
                        logger.debug(
                            f"Table {table_key}: {len(rows)} sample rows fetched"
                        )

            # 2.2 Populate column metadata from sampled rows in memory
            logger.info(
                "Processing column samples for {count} columns...", count=len(columns)
            )
            for col in columns:
                ...
                samples = crawler.build_column_samples_from_rows(
                    rows, col["column_name"]
                )
            ...
            tables = [
                t
                for t in tables
                if f"{t['schema_name']}.{t['table_name']}" in tables_with_samples
            ]
            columns = [
                c
                for c in columns
                if f"{c['schema_name']}.{c['table_name']}" in tables_with_samples
            ]
            ...
            for t in tables:
                ...
                t_cols = [
                    c
                    for c in columns
                    if f"{c['schema_name']}.{c['table_name']}" == table_key
                ]
            ...
            fks = [
                ...
                and f"{fk['referenced_schema_name']}.{fk['referenced_table_name']}"
                in tables_with_samples
            ]

            skipped_count = initial_table_count - len(tables)
            if skipped_count > 0:
                logger.info(
                    "Finished sampling. Kept {active} tables, skipped {skipped} tables (broken/empty/system).",
                    active=len(tables),
            ...
            async with self.session_factory() as session:
                async with session.begin():
                    ...
                    for schema_name in schemas:
                        schema_obj = await self._upsert_schema(
                            session, db_instance.id, schema_name
                        )
                    ...
                    for table in tables:
                        ...
                        table_obj = await self._upsert_table(
                            session, schema_id_map[s_name], table
                        )
                    ...
                    for col in columns:
                        ...
                        col_obj = await self._upsert_column(
                            session, table_id_map[table_key], col
                        )
                    ...
                    logger.info(
                        "Discovered {count} virtual foreign keys based on heuristics",
                        count=len(virtual_fks),
                    )
            ...
            logger.error(
                f"Schema extraction failed for {settings.sql_source_base}: {e}"
            )
            raise e

    async def _upsert_database(
        self, session: AsyncSession, req: SchemaIndexRequest
    ) -> Database:
        result = await session.execute(
            select(Database).where(
                Database.name == settings.sql_source_base,
                Database.host == settings.sql_source_host,
            )
        )
    ...
    async def _upsert_schema(
        self, session: AsyncSession, db_id, schema_name: str
    ) -> Schema:
        result = await session.execute(
            select(Schema).where(
                Schema.database_id == db_id, Schema.name == schema_name
            )
        )
    ...
    async def _upsert_table(
        self, session: AsyncSession, schema_id, table: dict[str, Any]
    ) -> Table:
    ...
    async def _upsert_column(
        self, session: AsyncSession, table_id, col: dict[str, Any]
    ) -> Column:
        result = await session.execute(
            select(Column).where(
                Column.table_id == table_id, Column.name == col["column_name"]
            )
        )
        ...
        for col in columns:
            ...
            if name_lower.endswith("_id"):
                ...
                for target_col in pk_candidates.get("id", []):
                    ...
                    if (
                        target_table_lower == base_name
                        or target_table_lower == base_name + "s"
                        or target_table_lower == base_name + "es"
                    ):
    ...
    async def _upsert_foreign_key(
        self, session: AsyncSession, col_id_map: dict[str, Any], fk: dict[str, str]
    ):
