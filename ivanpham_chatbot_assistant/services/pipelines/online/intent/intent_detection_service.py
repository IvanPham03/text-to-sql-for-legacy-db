...
class IntentDetectionService:
    ...
    async def detect(self, question: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Detected strategies: {valid_strategies}, requires_query: {requires_query}"
            )
            ...
            logger.error(
                f"Error during intent detection: {e}. Falling back to default strategy."
            )
            return {
                "status": "error",
                "strategies": [1],
