...
from ivanpham_chatbot_assistant.services.pipelines.online.intent.intent_detection_service import IntentDetectionService
from ivanpham_chatbot_assistant.services.pipelines.online.retrieval.schema_retrieval_service import SchemaRetrievalService
from ivanpham_chatbot_assistant.services.pipelines.online.generation.sql_generation_service import SqlGenerationService
from ivanpham_chatbot_assistant.services.pipelines.online.generation.sql_correction_service import SqlCorrectionService
from ivanpham_chatbot_assistant.services.pipelines.online.generation.result_refinement_service import ResultRefinementService
from ivanpham_chatbot_assistant.services.pipelines.online.generation.multi_step_planner_service import MultiStepPlannerService
from ivanpham_chatbot_assistant.services.pipelines.online.validation.sql_validation_service import SqlValidationService
from ivanpham_chatbot_assistant.services.pipelines.online.execution.sql_execution_service import SqlExecutionService
from ivanpham_chatbot_assistant.services.pipelines.online.answer.answer_generation_service import AnswerGenerationService
...
class OnlinePipeline:
    ...
    async def ask_question(self, request: AskRequest) -> AskResponseData:
        ...
        logger.debug("[step] intent_detection", latency_ms=round((time.perf_counter() - t0) * 1000, 2))

        if not intent_result.get("requires_query", True):
            logger.info(f"Intent detected as non-query (strategies: {intent_result.get('strategies')}). Generating direct answer.")
            t0 = time.perf_counter()
            answer_result = await self.answer_service.generate(question, [])
            logger.debug("[step] answer_generation", latency_ms=round((time.perf_counter() - t0) * 1000, 2))
            logger.info("[pipeline] done", total_ms=round((time.perf_counter() - pipeline_t0) * 1000, 2))
        ...
        # 2. Schema Retrieval
        logger.info(f"Intent requires query (strategies: {intent_result.get('strategies')}). Retrieving schema context.")
        t0 = time.perf_counter()
        retrieval_result = await self.retrieval_service.execute(question)
        logger.debug("[step] schema_retrieval", latency_ms=round((time.perf_counter() - t0) * 1000, 2))
        ...
        if 4 in strategies:
            logger.info("Complex query detected. Applying Strategy 4: Multi-Step Planning.")
        ...
        logger.debug("[step] sql_generation", latency_ms=round((time.perf_counter() - t0) * 1000, 2))

        # SCENARIO 1 — No schema was retrieved
        if gen_result.get("status") == "schema_missing":
            msg = gen_result.get("message", "No relevant database schema was retrieved. SQL generation skipped.")
            logger.warning("[pipeline] schema_missing – SQL generation skipped", question=question)
            logger.info("[pipeline] done (schema_missing)", total_ms=round((time.perf_counter() - pipeline_t0) * 1000, 2))
            return AskResponseData(
                answer=msg,
                sql=None,
                columns=None,
        ...
        if not generated_sql:
            logger.info("[pipeline] done (no SQL generated)", total_ms=round((time.perf_counter() - pipeline_t0) * 1000, 2))
            return AskResponseData(
    ...
    async def ask_question_stream(
        ...
        try:
            ...
            intent_result = await self.intent_service.detect(question)
            logger.debug("[step] intent_detection", latency_ms=round((time.perf_counter() - t0) * 1000, 2))
            yield {"event": "intent_detected", "strategies": intent_result.get("strategies")}

            if not intent_result.get("requires_query", True):
                logger.info(f"Intent detected as non-query (strategies: {intent_result.get('strategies')})")
                t0 = time.perf_counter()
                answer_result = await self.answer_service.generate(question, [])
                logger.debug("[step] answer_generation", latency_ms=round((time.perf_counter() - t0) * 1000, 2))
                logger.info("[stream] done", total_ms=round((time.perf_counter() - pipeline_t0) * 1000, 2))
                yield {
                    "event": "final_answer",
                    "answer": answer_result.get("answer", "I'm not sure how to answer that."),
                    "sql": None,
            ...
            # 2. Schema Retrieval
            logger.info(f"Intent requires query (strategies: {intent_result.get('strategies')})")
            t0 = time.perf_counter()
            retrieval_result = await self.retrieval_service.execute(question)
            retrieved_schema = retrieval_result.get("retrieved_schema", {})
            logger.debug("[step] schema_retrieval", latency_ms=round((time.perf_counter() - t0) * 1000, 2))
            ...
            if 4 in strategies:
                logger.info("Complex query detected. Applying Strategy 4: Multi-Step Planning.")
                yield {"event": "planning_steps", "message": "Complex query detected. Decomposing into logical steps..."}
                gen_result = await self.planner_service.plan_and_execute(
                    question, schema_context, None, None
                )
                generated_sql = gen_result.get("final_sql")
            ...
            if gen_result.get("status") == "schema_missing":
                msg = gen_result.get("message", "No relevant database schema was retrieved. SQL generation skipped.")
                logger.warning("[stream] schema_missing – SQL generation skipped", question=question)
            ...
            if correction_result["status"] != "success":
                ...
                logger.warning("[stream] self-correction exhausted", total_attempts=total_attempts)
                yield {
            ...
            yield {"event": "sql_validated", "is_valid": True, "total_attempts": total_attempts, "sql": final_sql}
            ...
            if 3 in strategies and correction_result.get("status") == "success" and final_data:
                ...
                if await self.refinement_service.should_refine(final_data, columns):
                    ...
                    yield {"event": "refining_result", "message": "Result needs more context. Fetching descriptive fields..."}
                    ...
                    if refine_result.get("status") == "success":
                        ...
                        if val_res.get("is_valid"):
                            ...
                            if exec_res.get("status") == "success":
                                ...
                                logger.warning(f"Refinement query execution failed: {exec_res.get('message')}")
                        else:
                            logger.warning(f"Refinement query validation failed: {val_res.get('reason')}")
            ...
            logger.info("[stream] done", total_ms=round((time.perf_counter() - pipeline_t0) * 1000, 2), row_count=len(rows))
            yield {
                "event": "final_answer",
                "answer": answer_result.get("answer", "No answer generated."),
                "sql": final_sql,
