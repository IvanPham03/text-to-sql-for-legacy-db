...
class SqlValidationService:
    ...
    async def validate(self, sql: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "error",
                "is_valid": False,
                "reason": f"Validation internal error: {e!s}",
            }
    ...
    def _parse_sql(self, sql: str) -> exp.Expression | None:
    ...
    def _extract_alias_map(
        self, expression: exp.Expression, ctes: set[str]
    ) -> dict[str, str]:
    ...
    def _resolve_columns(
        self, columns: list[dict[str, str | None]], alias_map: dict[str, str]
    ) -> list[dict[str, str | None]]:
        ...
        for col in columns:
            ...
            resolved.append({"table": table_ref, "column": col["column"]})
        return resolved

    async def _verify_metadata(
        self, tables: set[str], columns: list[dict[str, str | None]]
    ) -> dict[str, Any]:
        ...
        async with self.session_factory() as session:
            ...
            for db_table in db_tables:
                ...
                metadata[table_name] = (
                    cols  # Support lookup by just table name if unique
                )
            ...
            for s_name, t_name in table_lookups:
                if s_name:
                    ...
                    if full_name not in metadata:
                        return {
                            "valid": False,
                            "reason": f"Table '{full_name}' not found in metadata.",
                        }
                else:
                    if t_name not in metadata:
                        return {
                            "valid": False,
                            "reason": f"Table '{t_name}' not found in metadata.",
                        }
            ...
            for col in columns:
                ...
                if t_ref:
                    ...
                    if t_ref not in metadata:
                        return {
                            "valid": False,
                            "reason": f"Table/Alias '{t_ref}' referenced in column '{c_name}' not found.",
                        }
                    if c_name not in metadata[t_ref]:
                        return {
                            "valid": False,
                            "reason": f"Column '{c_name}' not found in table '{t_ref}'.",
                        }
                else:
                    ...
                    if found_in_count == 0:
                        return {
                            "valid": False,
                            "reason": f"Column '{c_name}' could not be resolved to any table in the query.",
                        }
    ...
    def _enforce_limit(
        self, expression: exp.Expression, limit: int = 100
    ) -> exp.Expression:
    ...
    async def safe_execute(
        ...
        async with self.session_factory() as session:
            ...
            await session.execute(
                sqlglot.transpile(
                    "SET TRANSACTION READ ONLY", read="postgres", write="postgres"
                )[0]
            )

            await session.execute(
                sqlglot.exp.select("*")
                .from_(sqlglot.exp.Table(name="sub", alias="sub"))
                .sql()
            )  # Dummy example
