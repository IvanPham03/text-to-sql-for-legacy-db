...
class QdrantService:
    ...
    async def upsert_vectors(self, points: list[dict[str, Any]], batch_size: int = 100):
        ...
        try:
            ...
            logger.info(
                f"Starting batched upsert of {total_points} points in {total_batches} batches..."
            )

            for idx, batch in enumerate(batches, 1):
                logger.info(
                    f"Upserting batch {idx}/{total_batches} ({len(batch)} vectors)"
                )
                await self._upsert_batch(list(batch))

            logger.info(
                f"Successfully upserted {total_points} vectors to {self.collection_name}"
            )
        except Exception as e:
