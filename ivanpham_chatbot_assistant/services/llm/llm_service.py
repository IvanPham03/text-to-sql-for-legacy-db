...
LLM_LATENCY = Histogram(
    "llm_latency_seconds", "Latency of LLM requests in seconds", ["provider", "model"]
)
LLM_TOKENS = Counter(
    "llm_tokens_total", "Total tokens consumed", ["provider", "model", "type"]
)
LLM_COST = Counter(
    "llm_cost_total", "Total estimated cost of LLM requests", ["provider", "model"]
)
...
class LLMService:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        if not self.provider_configs:
            ...
            if "name" in config:
                ...
                raise ValueError(
                    "LLM configuration must contain at least one provider."
                )
    ...
    async def chat(
            ...
            for attempt in range(3):
                ...
                try:
                    ...
                    response = await provider.llm.ainvoke(
                        provider._convert_messages(messages), **kwargs
                    )
                    ...
                    # Metrics
                    LLM_REQUESTS.labels(
                        provider=p_name, model=m_name, status="success"
                    ).inc()
                    LLM_LATENCY.labels(provider=p_name, model=m_name).observe(latency)
                    LLM_TOKENS.labels(provider=p_name, model=m_name, type="prompt").inc(
                        usage["prompt_tokens"]
                    )
                    LLM_TOKENS.labels(
                        provider=p_name, model=m_name, type="completion"
                    ...
                    LLM_FAILURES.labels(
                        provider=p_name, model=m_name, error_type=type(e).__name__
                    ).inc()
                    logger.warning(
                        f"LLM request failed (Attempt {attempt + 1}/3) via {p_name}: {e!s}"
                    )
                    last_exception = e
        ...
        raise RuntimeError(f"All LLM providers failed. Last error: {last_exception!s}")
    ...
    def _calculate_cost(self, model: str, usage: dict[str, int]) -> float:
        ...
        completion_cost = (usage["completion_tokens"] / 1_000_000) * model_pricing[
            "completion"
        ]
