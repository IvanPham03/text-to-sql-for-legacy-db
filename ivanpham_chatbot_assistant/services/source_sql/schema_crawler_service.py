...
class SchemaCrawlerService:
    ...
    def fetch_schemas(self) -> list[str]:
        ...
        excluded_schemas = ("information_schema", "sys", "guest", "db_owner", "db_accessadmin", 
        ...
        try:
            with self.engine.connect() as conn:
                ...
                schemas = [row[0] for row in result if row[0].lower() not in excluded_schemas]
                logger.info("Extracted {count} schemas after filtering", count=len(schemas))
                return schemas
    ...
    def fetch_columns(self) -> list[dict[str, Any]]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                logger.info("Extracted {count} columns after filtering", count=len(columns))
                return columns
        except Exception as e:
    ...
    def sample_table_rows(self, schema: str, table: str, limit: int | None = 100) -> list[dict[str, Any]]:
        """
        ...
        try:
            with self.engine.connect() as conn:
                ...
                if not rows:
                    ...
                    fallback = f"SELECT TOP {fallback_limit} * FROM {table_ref}" if self.db_type == "sqlserver" else f"SELECT * FROM {table_ref} LIMIT {fallback_limit}"
                    result = conn.execute(text(fallback).execution_options(timeout=20))
                    rows = [dict(row._mapping) for row in result]
                
                return rows
        except Exception as e:
            logger.warning("Skipping unreadable table {schema}.{table} during sampling: {error}",
                           schema=schema, table=table, error=str(e))
            return []
    ...
    def build_column_samples_from_rows(self, rows: list[dict[str, Any]], column_name: str) -> list[Any]:
        """
